...
def eval_meteor(instances: List[CFRInstance]):
    ...
    for r, h in tqdm.tqdm(zip(references, hypotheses)):
        ...
        # print(h)
        if len(h) == 0:
            meteor_score_total.append(0)
            continue
        else:
            meteor_score_total.append((meteor(r,h), 4)[0])
    ...
    metrics = {
        'mean_sentence_meteor': np.mean(meteor_score_total),
        'sentence_meteor_by_instance': meteor_score_total
    }
    return metrics
...
# def eval_semantic_sim_score(instances: List[CFRInstance], bert_model_type="./bert-base-uncased/bert_model.ckpt"):
...
def main(pred_endings_file, gold_file, bert_model):
    ...
    metrics = {}
    print("Eval METEOR ...")
    metrics.update(eval_meteor(instances))
    print("Eval BLEU ... ")
